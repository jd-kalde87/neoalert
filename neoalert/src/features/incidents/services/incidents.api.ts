...
export async function fetchIncidents(
  ...
  if (!USE_MOCK_API) {
    return authorizedRequest<Incident[]>(API_ENDPOINTS.incidents.list, {
...
export async function fetchIncidentById(id: string) {
  if (!USE_MOCK_API) {
    return authorizedRequest<Incident>(API_ENDPOINTS.incidents.detail(id))
  }
...
export async function createIncident(payload: CreateIncidentDto) {
  if (!USE_MOCK_API) {
    return authorizedRequest<Incident>(API_ENDPOINTS.incidents.create, {
      method: 'POST',
      body: payload,
    })
  }
...
export async function bulkCreateIncidentsFromImport(payloads: CreateIncidentDto[]) {
  if (!USE_MOCK_API) {
    const created: Incident[] = []
    for (const payload of payloads) {
      const incident = await authorizedRequest<Incident>(API_ENDPOINTS.incidents.create, {
        method: 'POST',
        body: payload,
      })
  ...
  const created: Incident[] = []
  let nextNumber = incidentsDb.length
  ...
  incidentsDb = [...created, ...incidentsDb]
  return created
}
...
export async function updateIncidentStatus(
  ...
  if (!USE_MOCK_API) {
    return authorizedRequest<Incident>(`${API_ENDPOINTS.incidents.detail(id)}/status`, {
      method: 'PATCH',
