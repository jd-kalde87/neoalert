...
export async function fetchAuditLogs(
  ...
  if (!USE_MOCK_API) {
    return authorizedRequest<AuditLogEntry[]>(API_ENDPOINTS.audit.list, {
...
export async function fetchAuditSummary(
  ...
  if (!USE_MOCK_API) {
    return authorizedRequest<AuditSummary>(`${API_ENDPOINTS.audit.list}/summary`, {
      params: toQueryParams({ ...globalFilters, ...localFilters }),
    })
  }
...
export async function fetchAuditById(id: string) {
  if (!USE_MOCK_API) {
    return authorizedRequest<AuditLogEntry>(API_ENDPOINTS.audit.detail(id))
  }
