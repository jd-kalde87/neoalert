...
import { parseDelimitedText, suggestMappings } from '../utils/parseFile'
...
export function ImportWizard() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setParseError('No se pudo leer el archivo. Use CSV delimitado por coma o punto y coma.')
    }
  }

  const goToPreview = () => {
    if (!fileName || headers.length === 0) {
      setParseError('Seleccione un archivo CSV válido.')
      return
    }
    if (!templateId) {
